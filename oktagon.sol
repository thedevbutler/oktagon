...
contract [CONTRACT_NAME] is Ownable, IERC20 {
    ...
    uint8 private constant FEE = 50; // = 5 %
    ...
    uint256 constant private MAX_TRANSACTION = 20000 * (10 ** DECIMALS); // 2 %
    uint256 constant private MAX_WALLET = 20000 * (10 ** DECIMALS); // 2 %
    ...
    function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 totalFees = 0;
        if (!fairExiting && !feesPaying && !excludedFromFees[sender] && !excludedFromFees[recipient]) {
